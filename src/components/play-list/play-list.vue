        ...
        <scroll class="list-content" ref="listContent" :data="sequenceList">
          <transition-group name="list" tag="ul">
            <li class="item" ref="listItem" @click="selectItem(item, index)" v-for="(item, index) in sequenceList" :key="item.id">
              ...
              <span class="like" @click.stop="toggleFavorite(item)"><i class="icon" :class="getFavoriteIcon(item)"></i></span>
...
.play-list{
  ...
  &.last-fade-enter-active, &.last-fade-leave-active{
    transition: opacity 0.3s;
    .list-wrapper{
      transition: all 0.3s;
    }
  }
  &.last-fade-enter, &.last-fade-leave-to{
    opacity: 0;
    .list-wrapper{
